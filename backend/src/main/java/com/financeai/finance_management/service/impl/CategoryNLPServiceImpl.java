...
public class CategoryNLPServiceImpl implements ICategoryNLPService {
    ...
    private void loadModel() {
        ...
        } catch (Exception e) { log.error("❌ Load Cat Model Fail"); }
    }
    ...
    public void trainMoneyModel() {
        try {
            ...
            ObjectStream<NameSample> sampleStream = new NameSampleDataStream(
                    new PlainTextByLineStream(new MarkableFileInputStreamFactory(res.getFile()), StandardCharsets.UTF_8));

            TrainingParameters params = new TrainingParameters();
            params.put(TrainingParameters.ITERATIONS_PARAM, 200);

            TokenNameFinderModel moneyModel = NameFinderME.train("vi", "money", sampleStream, params, new TokenNameFinderFactory());
            try (OutputStream out = new FileOutputStream("src/main/resources/nlp-models/money-model.bin")) {
                moneyModel.serialize(out);
            }
            OCRParser.reloadModel();
            log.info("✅ Trained Money Model");
        } catch (Exception e) { log.error("❌ Train Money Fail: {}", e.getMessage()); }
    }
