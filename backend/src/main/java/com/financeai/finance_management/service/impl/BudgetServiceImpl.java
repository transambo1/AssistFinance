...
public class BudgetServiceImpl implements IBudgetService {
  ...
  public BaseResponse<BudgetResponse> createBudget(BudgetCreationRequest request) {
    ...
    boolean existed =
        budgetRepository.existsByUserIdAndNameAndDeletedAtIsNull(
            currentUserId, request.getName().trim());
    ...
    Budget budget =
        Budget.builder()
            ...
            .status(resolveStatus(type, currentAmount, request.getTargetAmount(), true))
            .build();
  ...
  @Override
  @Transactional(readOnly = true)
  public BaseResponse<BasePaginationResponse<BudgetResponse>> getAllBudgets(
      BudgetFilterRequest request) {
    if (request == null) {
      throw new AppException(ErrorCode.INVALID_REQUEST);
    }
    ...
    Page<BudgetResponse> pageResponse =
        budgetRepository.findAll(specification, pageable).map(this::mapToResponse);
  ...
  public BaseResponse<String> activateBudget(String id) {
    ...
    budget.setActive(true);
    budget.setStatus(
        resolveStatus(budget.getType(), budget.getCurrentAmount(), budget.getTargetAmount(), true));
  ...
  private void validateCreateRequest(BudgetCreationRequest request) {
    ...
    if (request.getCurrentAmount() != null
        && request.getCurrentAmount().compareTo(BigDecimal.ZERO) < 0) {
      throw new AppException(ErrorCode.INVALID_REQUEST);
    }
  ...
  private void normalizeCreateDate(BudgetCreationRequest request) {
    ...
    if (request.getStartDate() == null && request.getEndDate() == null) {
      LocalDate end =
          today
              .plusMonths(request.getDurationMonths() != null ? request.getDurationMonths() : 1)
              .minusDays(1);
  ...
  private User findUserById(String userId) {
    return userRepository
        .findById(requireValidId(userId))
        .orElseThrow(() -> new AppException(ErrorCode.USER_NOT_EXISTED));
  }

  private Budget findBudgetById(String id) {
    return budgetRepository
        .findByIdAndDeletedAtIsNull(requireValidId(id))
        .orElseThrow(() -> new AppException(ErrorCode.DATASOURCE_NOT_FOUND));
  }
  ...
  private BudgetStatus resolveStatus(
      BudgetType type, BigDecimal currentAmount, BigDecimal targetAmount, boolean isActive) {
    if (!isActive) {
