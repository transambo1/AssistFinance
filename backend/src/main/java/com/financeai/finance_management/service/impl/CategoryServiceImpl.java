...
public class CategoryServiceImpl implements ICategoryService {
  ...
  public void createDefaultCategories(String userId) {
    ...
    List<Category> defaultCategories =
            List.of(
                    buildDefaultCategory(user, "Ăn uống", CategoryType.EXPENSE, "restaurant", "#FF6B6B"),
                    buildDefaultCategory(user, "Di chuyển", CategoryType.EXPENSE, "directions-car", "#4ECDC4"),
                    buildDefaultCategory(user, "Giải trí", CategoryType.EXPENSE, "movie", "#1A535C"),
                    buildDefaultCategory(user, "Lương", CategoryType.INCOME, "payments", "#2ECC71"),
                    buildDefaultCategory(user, "Thưởng", CategoryType.INCOME, "card-giftcard", "#27AE60"),
                    buildDefaultCategory(user, "Chi phí khác", CategoryType.EXPENSE, "category", "#9E9E9E"),
                    buildDefaultCategory(user, "Thu nhập khác", CategoryType.INCOME, "local-atm", "#9E9E9E"));
  ...
  public BaseResponse<CategoryResponse> createCategory(CategoryCreationRequest request) {
    ...
    boolean existed =
        categoryRepository.existsByUserIdAndNameAndType(currentUserId, name, request.getType());
  ...
  @Override
  @Transactional(readOnly = true)
  public BaseResponse<BasePaginationResponse<CategoryResponse>> getAllCategories(
    ...
    Page<CategoryResponse> pageResponse =
        categoryRepository.findAll(specification, pageable).map(this::mapToResponse);
  ...
  private Category buildDefaultCategory(
      User user, String name, CategoryType type, String icon, String color) {
