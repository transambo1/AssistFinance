...
public final class BudgetSpecification {
  ...
  public BudgetSpecification withUserId(String userId) {
    ...
    specifications.add(
        (root, query, cb) -> cb.equal(root.get(FIELD_USER).get(FIELD_ID), userId.trim()));
  ...
  public BudgetSpecification withCategoryId(String categoryId) {
    ...
    specifications.add(
        (root, query, cb) -> cb.equal(root.get(FIELD_CATEGORY_ID), categoryId.trim()));
  ...
  public BudgetSpecification withType(BudgetType type) {
    ...
    specifications.add((root, query, cb) -> cb.equal(root.get(FIELD_TYPE), type));
  ...
  public BudgetSpecification withStatus(BudgetStatus status) {
    ...
    specifications.add((root, query, cb) -> cb.equal(root.get(FIELD_STATUS), status));
  ...
  public BudgetSpecification withActive(Boolean isActive) {
    ...
    specifications.add((root, query, cb) -> cb.equal(root.get(FIELD_IS_ACTIVE), isActive));
  ...
  public BudgetSpecification withDateRange(Long startDate, Long endDate) {
    ...
    if (startDate != null) {
      specifications.add(
          (root, query, cb) -> cb.greaterThanOrEqualTo(root.get(FIELD_END_DATE), startDate));
    }

    if (endDate != null) {
      specifications.add(
          (root, query, cb) -> cb.lessThanOrEqualTo(root.get(FIELD_START_DATE), endDate));
    }
