...
export default function BudgetScreen() {
        ...
        Alert.alert(
            ...
            [
                { 
                    text: 'Thêm tiền', 
                    onPress: () => {
                        Alert.prompt('Thêm tiền', 'Nhập số tiền:', [
                                ...
                                onPress: (val: any) => {
                                    ...
                                    if (amount > 0) {
                                        updateMutation.mutate({ 
                                            id: item.id, 
                                            payload: { currentAmount: item.currentAmount + amount }
                                        });
                                    }
                                } 
                            }
    ...
    const handleUpdateBudget = () => {
        if (!editingBudget.targetAmount || editingBudget.targetAmount <= 0) {
                ...
                <View style={styles.modalOverlay}>
                    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.modalContent}>
                        <View style={styles.modalHeader}>
                            ...
                            <TouchableOpacity onPress={() => setEditModalVisible(false)}><MaterialIcons name="close" size={24} color="#1A237E" /></TouchableOpacity>
                        </View>
                        <ScrollView style={{ padding: 20 }}>
                            ...
                            <TextInput 
                                ...
                                // Hiển thị số tiền đã format khi không focus, hiển thị số thuần khi nhập
                                value={editingBudget.targetAmount?.toLocaleString('vi-VN')} 
...
const getStyles = (colors: any, isDark: boolean) => StyleSheet.create({
    ...
    dateText: { fontSize: 9, color: '#9CA3AF' },
    moreButton: { padding: 4 },
    progressBarBackground: { height: 8, backgroundColor: isDark ? '#333' : '#F3F4F6', borderRadius: 4, marginTop: 18 },
    progressBarFill: { height: '100%', borderRadius: 4 },
    modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' },
    modalContent: { backgroundColor: colors.surface, borderTopLeftRadius: 30, borderTopRightRadius: 30, height: '65%' },
